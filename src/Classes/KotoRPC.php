...
/**
 * KOTO RPC Client
 * Handles communication with KOTO daemon
 */
class KotoRPC
{
    private $config;
    ...
    public function __construct()
    {
        ...
        $kotoConfig = $this->config->getKotoConfig();
        
        $this->host = $kotoConfig['daemon_host'];
        $this->port = $kotoConfig['daemon_port'];
        $this->username = $kotoConfig['daemon_user'];
        $this->password = $kotoConfig['daemon_password'];
        $this->timeout = 30;
    }
    
    /**
     * Make RPC call to KOTO daemon
     */
    ...
    private function sendRequest($request)
    {
        ...
        if ($response === false) {
            throw new \Exception("Failed to connect to KOTO daemon");
        }
