...
/**
 * AdventureCoin RPC Client
 * Handles communication with AdventureCoin daemon
 * Uses YesPowerADVC algorithm
 */
class AdventureCoinRPC
{
    private $config;
    private $host;
    private $port;
    ...
    public function __construct()
    {
        ...
        $advcConfig = $this->config->getAdventureCoinConfig();
        
        $this->host = $advcConfig['daemon_host'];
        $this->port = $advcConfig['daemon_port'];
        $this->username = $advcConfig['daemon_user'];
        $this->password = $advcConfig['daemon_password'];
        $this->timeout = 30;
    }
    ...
    private function sendRequest($request)
    {
        ...
        if ($response === false) {
            throw new \Exception("Failed to connect to AdventureCoin daemon");
        }
    ...
    /**
     * Get algorithm info (YesPowerADVC specific)
     */
    public function getAlgorithmInfo()
    {
        return [
            'name' => 'YesPowerADVC',
            'description' => 'AdventureCoin uses YesPowerADVC algorithm',
