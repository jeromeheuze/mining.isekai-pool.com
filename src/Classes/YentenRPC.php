...
/**
 * Yenten RPC Client
 * Handles communication with Yenten daemon
 */
class YentenRPC
{
    private $config;
    ...
    public function __construct()
    {
        ...
        $yentenConfig = $this->config->getYentenConfig();
        
        $this->host = $yentenConfig['daemon_host'];
        $this->port = $yentenConfig['daemon_port'];
        $this->username = $yentenConfig['daemon_user'];
        $this->password = $yentenConfig['daemon_password'];
        $this->timeout = 30;
    }
    
    /**
     * Make RPC call to Yenten daemon
     */
    ...
    private function sendRequest($request)
    {
        ...
        if ($response === false) {
            throw new \Exception("Failed to connect to Yenten daemon");
        }
